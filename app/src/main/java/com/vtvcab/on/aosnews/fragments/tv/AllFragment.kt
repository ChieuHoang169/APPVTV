...
class AllFragment : BaseFragment(), View.OnClickListener, View.OnTouchListener {
    @SuppressLint("ClickableViewAccessibility")
    override fun onTouch(view: View, motionEvent: MotionEvent): Boolean {
        if (view.id == R.id.videoPlayerAll) {
            when (motionEvent.action) {
                ...
                MotionEvent.ACTION_UP -> {
                    ...
                    if (includeAll.visibility == View.INVISIBLE) {
                        showController()
    ...
    override fun setLayoutId(): Int = R.layout.fragment_all
    ...
    override fun initListener() {
        ...
        videoPlayerAll.setOnTouchListener(this)
        failedViewAll.setOnClickListener(this)
    }

    private val playbackObservable = Observable.interval(1, TimeUnit.SECONDS)
        .map { videoPlayerAll?.player?.currentPosition }
        .observeOn(AndroidSchedulers.mainThread())

    override fun bindData() {
        failedViewAll?.visibility = View.GONE
        loadingViewAll?.visibility = View.VISIBLE
        shimmer_view_containerAll?.visibility = View.VISIBLE
        flShimmerAll?.visibility = View.VISIBLE
        shimmer_view_containerAll?.startShimmer()
    ...
    override fun setUserVisibleHint(isVisibleToUser: Boolean) {
        if (isVisibleToUser){
            videoPlayerAll?.visibility = View.VISIBLE
            failedViewAll?.visibility = View.GONE
            loadingViewAll?.visibility = View.VISIBLE
            shimmer_view_containerAll?.visibility = View.VISIBLE
            flShimmerAll?.visibility = View.VISIBLE
            ...
            videoPlayerAll?.visibility = View.GONE
            videoPlayerAll?.player?.playWhenReady = false
        }
    ...
    private fun setPlayVideoDX() {
        ...
        videoPlayerAll?.setVideoUri(mUrl, "")

        videoPlayerAll?.player?.addListener(playerListener)
    }

    private fun callAPIWatch(url: String) {
     /*   NewsApplication.getApiService().getUrl(url).enqueue(object : Callback<String> {
            override fun onFailure(call: Call<String>, t: Throwable) {
                failedViewAll.visibility = View.VISIBLE
                shimmer_view_containerAll.stopShimmer()
                shimmer_view_containerAll.visibility = View.GONE
            }

            override fun onResponse(call: Call<String>, response: Response<String>) {
                if (response.isSuccessful) {
                    if (response.body() != null) {
                        webDetailAll?.loadDataWithBaseURL(null, response.body(), "text/html", "UTF-8", null)
                    } else {
                        failedViewAll.visibility = View.VISIBLE
                        shimmer_view_containerAll.stopShimmer()
                        shimmer_view_containerAll.visibility = View.GONE
        ...
        webDetailAll?.loadUrl("https://dothocam.000webhostapp.com/category/lineTV.html")
    }

    @SuppressLint("SetJavaScriptEnabled")
    private fun setupWebview() {
        val webSetting = webDetailAll.settings
        webSetting.javaScriptEnabled = true
        webDetailAll.webChromeClient = detailChromeClient
        webDetailAll.webViewClient = webViewClient
    }
...
//        pushFragment(R.id.homeContainerTH, DetailFragment(), bundle, true)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 1 && resultCode == Activity.RESULT_OK && data != null) {
            videoPlayerAll?.player?.seekTo(data.getLongExtra("currentPosition", videoPlayerAll.player.duration))
        }
    }
    ...
    private val webViewClient = object : WebViewClient() {
        override fun onPageFinished(view: WebView?, url: String?) {
            Handler().postDelayed({
                if (shimmer_view_containerAll != null) {
                    shimmer_view_containerAll.stopShimmer()
                    shimmer_view_containerAll.visibility = View.GONE
                    flShimmerAll.visibility = View.GONE
                }
                if (webDetailAll != null) {
                    webDetailAll.visibility = View.VISIBLE
                }
    ...
    private val playerListener = object : Player.DefaultEventListener() {
        override fun onPlayerStateChanged(playWhenReady: Boolean, playbackState: Int) {
            if (playbackState == Player.STATE_READY) {
                loadingViewAll?.visibility = View.GONE
                videoPlayerAll?.visibility = View.VISIBLE
        ...
        playbackDisposable = playbackObservable.subscribe { progress ->
            if (progress != null) {
                progress_bar_videoDX.progress =
                    ((progress.toFloat() / videoPlayerAll.player.duration.toFloat()) * 100).toInt()

                tvCurrentTimeDX.text = convertTimeFormat(progress)
                tvDurationDX.text = convertTimeFormat(videoPlayerAll.player.duration)
            }
        }
    }
    ...
    override fun onClick(view: View) {
        when (view) {
            imgPlayDX -> {
                if (videoPlayerAll != null) {
                    if (isPlaying()) {
                        imgPlayDX.setImageResource(R.drawable.ic_play)
                        videoPlayerAll.player.playWhenReady = false
                    } else {
                        imgPlayDX.setImageResource(R.drawable.ic_pause)
                        videoPlayerAll.player.playWhenReady = true
                    }
    ...
    private fun hideController() {
        includeAll?.visibility = View.INVISIBLE
    }

    private fun showController() {
        includeAll?.visibility = View.VISIBLE
    }
    ...
    private fun isPlaying(): Boolean {
        return if (videoPlayerAll != null) {
            val player = videoPlayerAll.player
            (player != null
