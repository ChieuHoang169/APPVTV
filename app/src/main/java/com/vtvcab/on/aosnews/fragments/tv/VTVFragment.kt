...
import kotlinx.android.synthetic.main.fragment_vtv.*
import kotlinx.android.synthetic.main.layout_media_controller_watch.*
import java.text.SimpleDateFormat
import java.util.*
import java.util.concurrent.TimeUnit

class VTVFragment : BaseFragment(), View.OnClickListener, View.OnTouchListener {
    @SuppressLint("ClickableViewAccessibility")
    override fun onTouch(view: View, motionEvent: MotionEvent): Boolean {
        if (view.id == R.id.videoPlayerVTV) {
            when (motionEvent.action) {
                ...
                MotionEvent.ACTION_UP -> {
                    ...
                    if (includeVTV.visibility == View.INVISIBLE) {
                        showController()
    ...
    override fun setLayoutId(): Int = R.layout.fragment_vtv
    ...
    override fun initListener() {
        ...
        videoPlayerVTV.setOnTouchListener(this)
        failedViewVTV.setOnClickListener(this)
    }

    private val playbackObservable = Observable.interval(1, TimeUnit.SECONDS)
        .map { videoPlayerVTV?.player?.currentPosition }
    ...
    override fun setUserVisibleHint(isVisibleToUser: Boolean) {
        if (isVisibleToUser){
            videoPlayerVTV?.visibility = View.VISIBLE
            failedViewVTV?.visibility = View.GONE
            loadingViewVTV?.visibility = View.VISIBLE
            shimmer_view_containerVTV?.visibility = View.VISIBLE
            flShimmerVTV?.visibility = View.VISIBLE
            shimmer_view_containerVTV?.startShimmer()

            setPlayVideoDX()
            callAPIWatch("lineVTV.html")
        } else{
            videoPlayerVTV?.visibility = View.GONE
            videoPlayerVTV?.player?.playWhenReady = false
...
//            Toast.makeText(baseActivity, "onPauseFragment():" + "VTVFragmen", Toast.LENGTH_SHORT).show();
    ...
    private fun setPlayVideoDX() {
        ...
        videoPlayerVTV?.setVideoUri(mUrl, "")

        videoPlayerVTV?.player?.addListener(playerListener)
    }

    private fun callAPIWatch(url: String) {/*
        NewsApplication.getApiService().getUrl(url).enqueue(object : Callback<String> {
            override fun onFailure(call: Call<String>, t: Throwable) {
                failedViewVTV.visibility = View.VISIBLE
                shimmer_view_containerVTV.stopShimmer()
                shimmer_view_containerVTV.visibility = View.GONE
            }

            override fun onResponse(call: Call<String>, response: Response<String>) {
                if (response.isSuccessful) {
                    if (response.body() != null) {
                        webDetailVTV?.loadDataWithBaseURL(null, response.body(), "text/html", "UTF-8", null)
                    } else {
                        failedViewVTV.visibility = View.VISIBLE
                        shimmer_view_containerVTV.stopShimmer()
                        shimmer_view_containerVTV.visibility = View.GONE
        ...
        webDetailVTV!!.loadUrl("https://dothocam.000webhostapp.com/category/lineVTV.html")
    }

    @SuppressLint("SetJavaScriptEnabled")
    private fun setupWebview() {
        val webSetting = webDetailVTV.settings
        webSetting.javaScriptEnabled = true
        webDetailVTV.webChromeClient = detailChromeClient
        webDetailVTV.webViewClient = webViewClient
    }
...
//        pushFragment(R.id.homeContainerTH, DetailFragment(), bundle, true)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 1 && resultCode == Activity.RESULT_OK && data != null) {
            videoPlayerVTV?.player?.seekTo(data.getLongExtra("currentPosition", videoPlayerVTV.player.duration))
        }
    }
    ...
    private val webViewClient = object : WebViewClient() {
        override fun onPageFinished(view: WebView?, url: String?) {
            Handler().postDelayed({
                if (shimmer_view_containerVTV != null) {
                    shimmer_view_containerVTV.stopShimmer()
                    shimmer_view_containerVTV.visibility = View.GONE
                    flShimmerVTV.visibility = View.GONE
                }
                if (webDetailVTV != null) {
                    webDetailVTV.visibility = View.VISIBLE
                }
    ...
    private val playerListener = object : Player.DefaultEventListener() {
        override fun onPlayerStateChanged(playWhenReady: Boolean, playbackState: Int) {
            if (playbackState == Player.STATE_READY) {
                loadingViewVTV?.visibility = View.GONE
                videoPlayerVTV?.visibility = View.VISIBLE
        ...
        playbackDisposable = playbackObservable.subscribe { progress ->
            if (progress != null) {
                progress_bar_videoDX.progress =
                    ((progress.toFloat() / videoPlayerVTV.player.duration.toFloat()) * 100).toInt()

                tvCurrentTimeDX.text = convertTimeFormat(progress)
                tvDurationDX.text = convertTimeFormat(videoPlayerVTV.player.duration)
            }
        }
    }
    ...
    override fun onClick(view: View) {
        when (view) {
            imgPlayDX -> {
                if (videoPlayerVTV != null) {
                    if (isPlaying()) {
                        imgPlayDX.setImageResource(R.drawable.ic_play)
                        videoPlayerVTV.player.playWhenReady = false
                    } else {
                        imgPlayDX.setImageResource(R.drawable.ic_pause)
                        videoPlayerVTV.player.playWhenReady = true
                    }
    ...
    private fun hideController() {
        includeVTV?.visibility = View.INVISIBLE
    }

    private fun showController() {
        includeVTV?.visibility = View.VISIBLE
    }
    ...
    private fun isPlaying(): Boolean {
        return if (videoPlayerVTV != null) {
            val player = videoPlayerVTV.player
            (player != null
